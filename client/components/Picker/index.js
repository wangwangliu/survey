...
class PickerColumn extends Component {
  ...
  computeTranslate = (props) => {
    ...
    if (selectedIndex < 0) {
      ...
      this.onValueSelected(options[0],1);
      selectedIndex = 0;
    }
    return {
...
export class BirthPicker extends Component {
  constructor(props) {
    super(props);
    const defaultDate = props.defaultDate?props.defaultDate.split('-'):[];
    const year = defaultDate[0]||new Date().getFullYear();
    const month = defaultDate[1]||new Date().getMonth()+1;
    const day = defaultDate[2]||new Date().getDate();
    this.state = {
      isPickerShow: props.show,
      valueGroups: {
        year: generateNumber(year),
        month: generateNumber(month),
        day: generateNumber(day)
      },
      optionGroups: {
        year: generateNumberArray(1970, new Date().getFullYear()),
  ...
  togglePicker = (item) => {
    ...
    // }),()=>{
      if(item=='ok'){
        this.props.handleChange({...valueGroups})
        return
      }
      ...
      <div className="modal-container">
        <div className="picker-modal-container">
          ...
          <div className={pickerModalClass}>
            <header>
              <div className="modal_cancel" onClick={this.togglePicker}>{props.textStyle.cancel||'cancel'}</div>
              <div className="modal_ok" onClick={this.togglePicker.bind(this,'ok')}>{props.textStyle.ok||'OK'}</div>
            </header>
            <Picker
